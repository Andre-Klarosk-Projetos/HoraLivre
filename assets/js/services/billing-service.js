...
import { getMonthReference, getMonthNumberFromReference } from '../utils/date-utils.js';
...
function buildBillingSettingsPayload(tenantId, data = {}) {
  return {
    ...
    annualBillingMonth: data.annualBillingMonth
      ? toNumber(data.annualBillingMonth, 0)
      : null,
...
function buildBillingRecordPayload(data = {}) {
  const monthRef = normalizeMonthReference(
    data.monthRef || data.reference || getMonthReference()
  );
...
export async function createBillingRecordForTenant(tenantId, monthReference, data = {}) {
  ...
  /* CORREÇÃO MÍNIMA:
     mantém monthRef em YYYY/MM nos dados,
     mas remove "/" do ID do documento para não quebrar o Firestore. */
  const safeMonthRefForId = monthRef.replace(/\//g, '-');
  const recordId = `billing_${safeMonthRefForId}_${tenantId}`;
...
export async function updateBillingRecord(recordId, data = {}) {
  ...
  await updateDoc(reference, {
    ...
    ...(data.fixedAmount !== undefined
      ? { fixedAmount: toNumber(data.fixedAmount, 0) }
      : {}),
    ...(data.fixedMonthlyPrice !== undefined
      ? { fixedMonthlyPrice: toNumber(data.fixedMonthlyPrice, 0) }
      : {}),
    ...(data.annualAmount !== undefined
      ? { annualAmount: toNumber(data.annualAmount, 0) }
      : {}),
    ...(data.annualPrice !== undefined
      ? { annualPrice: toNumber(data.annualPrice, 0) }
      : {}),
    ...
    ...(data.notes !== undefined
      ? { notes: data.notes || '' }
      : {}),
    ...(data.companyNameSnapshot !== undefined
      ? { companyNameSnapshot: data.companyNameSnapshot || '' }
      : {}),
    ...(data.companyWhatsappSnapshot !== undefined
      ? { companyWhatsappSnapshot: data.companyWhatsappSnapshot || '' }
      : {}),
    ...(data.planIdSnapshot !== undefined
      ? { planIdSnapshot: data.planIdSnapshot || '' }
      : {}),
...
export async function markBillingRecordAsPending(recordId) {
  await updateBillingRecord(recordId, { status: 'pending' });
}
